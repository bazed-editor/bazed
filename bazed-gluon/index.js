...
const serverBinaryPath = join(__dirname, '..', 'target', 'debug', 'bazed' + (process.platform === 'win32' ? '.exe' : ''));
const serverProc = spawn(serverBinaryPath, {
  stdio: 'inherit'
});

// Start local server for frontend (shouldn't be needed in future)
const rekuhoBuildPath = join(__dirname, '..', 'rekuho', 'build');
